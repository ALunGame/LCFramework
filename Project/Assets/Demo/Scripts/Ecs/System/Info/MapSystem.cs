...
namespace Demo.System
{
    public class MapSystem:BaseSystem
    {
        ...
        protected override void HandleComs(List<BaseCom> comList)
        {
            ...
            Vector2 playerPos = ECSLayerLocate.Info.GetSensor<PlayerSensor>(SensorType.Player).GetPlayerPos();
            RefreshMap(playerPos);
            UpdatePlayerMapPos(playerPos);
        }
        ...
        private void CreateGoEntity(Transform goRoot,List<GoInfo> goInfos, List<Entity> entities, Vector2Int mapPos)
        {
            ...
            for (int i = 0; i < goInfos.Count; i++)
            {
                ...
                entity = ECSLocate.ECS.CreateEntity(info.GoEntityConfId, childgo);
                entity.Disable();
