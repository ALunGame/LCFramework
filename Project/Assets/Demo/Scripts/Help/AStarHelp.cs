...
namespace Demo.Help
{
    ...
    public static class AstarHelp
    {
        ...
        private static bool FindNextPathNode(PathNode parentNode)
        {
            ...
            for (int i = -1; i <= 1; i++)
            {
                for (int j = -1; j <= 1; j++)
                {
                    ...
                    //0,检测下一步是否超出地图边界或者已经加入closeList或者是不可到达状态。
                    if (nextPos.x >= MapSize.x || nextPos.y >= MapSize.y || nextPos.x < 0 || nextPos.y < 0)
                        continue;
                    
                    //1,检测下一步是否加入closeList或者是不可到达状态。
        ...
        public static List<Vector2> FindPath(Vector2 startPos, Vector2 endPos, List<Vector2> obstaclePos, Vector2 offVect,List<Vector2> roadPath = null)
        {
            lock (LockObj)
            {
                Vector2Int tmpstartPos          = new Vector2Int((int)(startPos.x+ offVect.x), (int)(startPos.y + offVect.y));
                Vector2Int tmpendPos            = new Vector2Int((int)(endPos.x  + offVect.x), (int)(endPos.y   + offVect.y));
                List<Vector2Int> tmpobstaclePos = new List<Vector2Int>();
                List<Vector2Int> tmproadPath    = null;

                //位置转换
                for (int i = 0; i < obstaclePos.Count; i++)
                {
                    Vector2 pos     = obstaclePos[i];
                    tmpobstaclePos.Add(new Vector2Int((int)(pos.x + offVect.x), (int)(pos.y + offVect.y)));
                }
                if (roadPath != null && roadPath.Count > 0)
                {
                    tmproadPath = new List<Vector2Int>();
                    for (int i = 0; i < roadPath.Count; i++)
                    {
                        Vector2 pos = roadPath[i];
                        tmproadPath.Add(new Vector2Int((int)(pos.x + offVect.x), (int)(pos.y + offVect.y)));
                    }
                }

                List<Vector2Int> pathList = FindPath(tmpstartPos, tmpendPos, tmpobstaclePos, tmproadPath);
                //位置转换
                List<Vector2> resPathList = new List<Vector2>();
                for (int i = 0; i < pathList.Count; i++)
                {
                    Vector2Int pos = pathList[i];
                    resPathList.Add(new Vector2(pos.x- offVect.x, pos.y - offVect.y));
                }
