...
namespace LCSkill
{
    ...
    public class SkillCom : BaseCom
    {
        ...
        internal bool LearnSkill(SkillModel skillModel, int level = 1)
        {
            ...
            {
                for (int i = 0; i < skillModel.addBuffs.Length; i++)
                {
        ...
        internal bool ReleaseSkill(int skillId)
        {
            ...
            {
                timeline = BuffFreedFunc.ExecuteFunc(buffs[i].model.onFreedFunc, buffs[i], skillObj, timeline);
            }
            if (timeline == null)
            {
                return false;
        ...
        internal void AddBuff(AddBuffInfo addBuffInfo)
        {
            ...
            else
            {
                ...
                BuffOccurFunc.ExecuteFunc(addBuffInfo.buffModel.onOccurFunc, newBuff, addStack);
            }
        }
        ...
        private void UpdateBuff(AddBuffInfo addBuffInfo, List<BuffObj> buffs)
        {
            ...
            for (int i = 0; i < buffs.Count; i++)
            {
                ...
                if (buffObj.stack > 0)
                {
                    BuffOccurFunc.ExecuteFunc(addBuffInfo.buffModel.onOccurFunc, buffObj, addStack);
                }
                else
                {
                    //设置持续为0，下一帧移除
