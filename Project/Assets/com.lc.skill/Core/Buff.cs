...
namespace LCSkill
{
    ...
    public struct BuffModel 
    {
        ...
        /// 当Buff被添加、改变层数时执行的函数
        /// </summary>
        public string onOccurFunc;

        /// <summary>
        /// Buff在tickTime间隔执行的函数
        /// </summary>
        public string onTickFunc;

        /// <summary>
        /// Buff将要被移除时执行的函数
        /// </summary>
        public string onRemovedFunc;

        /// <summary>
        /// 当释放一个技能时执行的函数
        /// 为了处理当技能释放时，更改技能表现，比如没有魔法还要释放，会执行没有魔法的动画
        /// </summary>
        public string onFreedFunc;

        /// <summary>
        /// 在执行伤害流程时，拥有这个Buff作为攻击者执行的函数
        /// </summary>
        public string onHitFunc;

        /// <summary>
        /// 在执行伤害流程时，拥有这个Buff作为挨打者执行的函数
        /// </summary>
        public string onBeHurtFunc;

        /// <summary>
        /// 在执行伤害流程时，如果击杀目标执行的函数
        /// </summary>
        public string onKillFunc;

        /// <summary>
        /// 在执行伤害流程时，拥有这个Buff被杀死执行的函数
        /// </summary>
        public string onBeKilledFunc;
    }
