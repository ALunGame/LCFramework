...
namespace LCTimeline.Serialize
{
    /// <summary>
    ///  Timeline序列化
    /// </summary>
    public static class TimelineSerialize
    {
        ...
        public static void Save(TimelineData data, string savePath)
        {
        ...
        public static TimelineData Load(string filePath)
        {
            if (string.IsNullOrEmpty(filePath) || !File.Exists(filePath))
                return null;
            string jsonStr = File.ReadAllText(filePath);
            return JsonMapper.ToObject<TimelineData>(jsonStr);
        }
    }
}