...
namespace LCTimeline.View
{
    ...
    public class TimeAreaView : BaseView
    {
        ...
        public override void OnDraw()
        {
            ...
            timeAreaShow.TimeRuler(TimeHeaderRect, window.FrameRate, true, false, 1.0f, window.TimeInFrames ? TimeAreaShowHelper.TimeFormatStyle.Frame : TimeAreaShowHelper.TimeFormatStyle.TimeFrame);
        ...
        public double SnapToFrameIfRequired(double time)
        {
            ...
            if (window.FrameSnap)
            {
                result = TimeUtility.FromFrames(TimeUtility.ToFrames(time, (double)window.FrameRate), (double)window.FrameRate);
            }
            else
            {
                result = time;
